...
// --- FIREBASE SETUP ---
// Ensure your Firebase config is valid in the environment variables or replaced here
...
const LoginScreen = ({ onLogin, error }) => {
  ...
  const handleGuestLogin = async () => {
    ...
    try {
        ...
        if (err.code === 'auth/operation-not-allowed') {
            msg = "Guest login is disabled in Firebase Console (Auth > Sign-in method > Anonymous).";
        }
        onLogin(null, msg);
    } finally {
        setIsLoggingIn(false);
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-gray-900 rounded-2xl shadow-2xl p-8 border border-gray-800 relative overflow-hidden">
        ...
        <div className="relative z-10">
            ...
            <div className="p-4 bg-green-500/10 rounded-full ring-1 ring-green-500/30 shadow-[0_0_20px_rgba(34,197,94,0.2)]">
                <Activity className="w-12 h-12 text-green-500" />
            </div>
            </div>
            <h1 className="text-3xl font-bold text-center text-white mb-2 tracking-tight">ZeroSum Trader</h1>
            <p className="text-gray-400 text-center mb-8">Real-time high-frequency trading simulation.</p>
            ...
            <button
                ...
                className="w-full bg-white hover:bg-gray-100 text-gray-900 font-bold py-3.5 rounded-xl transition-all transform hover:scale-[1.02] flex items-center justify-center gap-3 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
            >
                {isLoggingIn ? (
            ...
            <button
                ...
                disabled={isLoggingIn}
                className="w-full bg-gray-800 hover:bg-gray-700 text-gray-300 font-bold py-3.5 rounded-xl transition-all border border-gray-700 disabled:opacity-50 flex items-center justify-center gap-2"
            >
                {isLoggingIn ? (
                    <>
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-4 relative">
      <div className="absolute top-6 right-6">
        <button onClick={onSignOut} className="text-gray-500 hover:text-white text-sm transition-colors px-4 py-2 rounded-lg hover:bg-gray-900 border border-transparent hover:border-gray-800">Sign Out</button>
      </div>
...
// A. Enhanced Stock Chart (Area Chart with Gradients)
const StockChart = ({ history }) => {
  ...
  const pointsArray = history.map((h, i) => {
    const x = ((h.time - startTime) / totalTime) * width;
    const y = height - ((h.price - effectiveMin) / priceRange) * height;
    return {x, y};
  });
  ...
  // Area string (close the loop at the bottom)
  const areaPoints = `0,${height} ${linePoints} ${width},${height}`;

  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden">
        {/* Y-Axis Labels */}
        <div className="absolute inset-y-0 right-0 flex flex-col justify-between pointer-events-none text-[10px] font-mono text-gray-500 py-2 select-none z-10 opacity-50">
        ...
        {width > 0 && (
            <svg width="100%" height="100%" className="overflow-visible">
                ...
                <line x1="0" y1={height * 0.25} x2={width} y2={height * 0.25} stroke="#374151" strokeDasharray="4 4" strokeWidth="1" opacity="0.2" />
                <line x1="0" y1={height * 0.50} x2={width} y2={height * 0.50} stroke="#374151" strokeDasharray="4 4" strokeWidth="1" opacity="0.2" />
                <line x1="0" y1={height * 0.75} x2={width} y2={height * 0.75} stroke="#374151" strokeDasharray="4 4" strokeWidth="1" opacity="0.2" />

                {/* Area Fill */}
                <path 
                    d={`M${pointsArray[0].x},${height} L${linePoints} L${pointsArray[pointsArray.length-1].x},${height} Z`} 
                ...
                <circle cx={pointsArray[pointsArray.length-1].x} cy={pointsArray[pointsArray.length-1].y} r="3" fill={color} />
                <circle cx={pointsArray[pointsArray.length-1].x} cy={pointsArray[pointsArray.length-1].y} r="8" fill={color} opacity="0.3">
                ...
                {playerArray.map(p => {
                    ...
                    return (
                        <div key={p.id} className="flex flex-col items-center flex-1 h-full justify-end group">
                            ...
                            <div 
                                className="w-full max-w-[60px] bg-blue-600 rounded-t-md hover:bg-blue-500 transition-all relative shadow-[0_0_10px_rgba(37,99,235,0.3)]"
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-8">
                        ...
                        <div>
                            ...
                            <div className={`text-3xl font-mono font-bold tracking-tight flex items-center gap-2 ${room.history.length > 1 && room.price >= room.history[room.history.length-2].price ? 'text-emerald-500' : 'text-red-500'}`}>
                                {formatCurrency(room.price)}
                                {room.history.length > 1 && room.price >= room.history[room.history.length-2].price ? <TrendingUp className="w-6 h-6" /> : <TrendingDown className="w-6 h-6" />}
                            </div>
                        </div>
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      if (u) {
        ...
        // If we are currently in login screen, move to lobby
